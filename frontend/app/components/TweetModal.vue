...
const { user } = useUser()
const newComment = defineModel<string>('newComment')
const isOwner = computed(() => Number(props.tweet?.userId) === Number(user.value?.id))
      ...
      <div v-if="tweet">
        ...
        <div class="flex items-center justify-between px-6 py-4 border-b border-base-200">
          <div class="flex items-center gap-3">
            <AvatarIcon :username="tweet.user?.username ?? user_${tweet.userId}" size="md" />
            <div>
              <p class="font-bold text-sm">{{ tweet.user?.username || User #${tweet.userId} }}</p>
              <p class="text-xs opacity-40">@{{ tweet.user?.username?.toLowerCase() || user_${tweet.userId} }}</p>
            </div>
          </div>
        ...
        <div class="px-6 py-5 space-y-5">
          ...
          <div v-if="isEditing">
            ...
            <div class="flex gap-2 mt-2">
              <button @click="confirmEdit" class="btn btn-sm btn-success">Speichern</button>
              <button @click="isEditing = false" class="btn btn-sm btn-ghost">Abbrechen</button>
            </div>
          </div>
...
<style scoped>
.max-h-52::-webkit-scrollbar { width: 4px; }
.max-h-52::-webkit-scrollbar-thumb { background: hsl(var(--bc) / 0.2); border-radius: 10px; }
</style>