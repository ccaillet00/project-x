...
definePageMeta({ layout: "app-default", middleware: "auth", path: "/profile" });
...
<template>
  <div class="container mx-auto mt-8 max-w-2xl px-4">
    <!-- Profil Header -->
    <div
      class="card bg-base-100 border border-base-300 rounded-2xl p-6 mb-6 flex flex-row items-center gap-5"
    >
      <AvatarIcon :username="user?.username ?? ''" size="lg" />
      <div>
        <h1 class="text-2xl font-bold">{{ user?.username }}</h1>
        <p class="opacity-40 text-sm">
          @{{ user?.username?.toLowerCase() }} · {{ posts?.length || 0 }} Posts
        </p>
      </div>
    </div>

    <!-- Posts -->
    <h2 class="text-sm uppercase tracking-widest opacity-40 mb-3 px-1">
      Meine Posts
    </h2>
